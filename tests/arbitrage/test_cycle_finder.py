...
def mock_binance_client():
    ...
    with patch('cli_monitor.arbitrage.cycle_finder.BinanceClient', new_callable=AsyncMock) as mock_client:
        yield mock_client

@pytest.fixture
def mock_config():
    """Фікстура для мокування об'єкта конфігурації."""
    with patch('cli_monitor.arbitrage.cycle_finder.config') as mock_config:
...
@pytest.mark.asyncio
async def test_get_trading_pairs(mock_config, mock_binance_client):
    ...
    pairs = await finder._get_trading_pairs()
...
@pytest.mark.asyncio
async def test_find_cycles_dfs(mock_config, mock_binance_client):
    """Тестує алгоритм пошуку циклів (DFS)."""
    # Arrange
    finder = await CycleFinder.create()
    finder.trading_pairs = {
        'USDT': ['BTC', 'BNB'],
        'BTC': ['USDT', 'ETH'],
        'ETH': ['BTC'],
        'BNB': ['USDT']
    ...
    # Act
    cycles = finder._find_cycles_dfs(finder.trading_pairs, 'USDT', 3)
...
async def test_save_cycles(mock_open, mock_config, mock_binance_client):
...
async def test_run(mock_save_cycles, mock_config, mock_binance_client):
    """Тестує головний метод `run`."""
    # Arrange
    mock_binance_client.create.return_value.get_exchange_info.return_value = {
        'symbols': [
            {'baseAsset': 'BTC', 'quoteAsset': 'USDT', 'status': 'TRADING'},
            {'baseAsset': 'ETH', 'quoteAsset': 'BTC', 'status': 'TRADING'},
            {'baseAsset': 'BNB', 'quoteAsset': 'USDT', 'status': 'TRADING'},
